...
rubrica::rubrica(unsigned int cap)
	: _voci(0), _capacity(0), _count(0) {
...
unsigned int rubrica::get_capacity(void) const {
	return _capacity;
}

unsigned int rubrica::get_count(void) const {
	return _count;
}
...
void rubrica::set_voce(unsigned int index, const voce &value) {
	...
	// Esempio di controllo della voce prima di inserirla
	for (unsigned int i=0; i<_count; ++i)
		if ((value.ntel == _voci[i].ntel) &&
				(value.nome != _voci[i].nome ||
...
void rubrica::add(const voce &v) {
	if (_count == _capacity)
		throw rubrica_full_exception();

	for(unsigned int i = 0; i< _count; ++i) {
		if(v==_voci[i])
			throw voce_duplicated_exception();
	}
...
void rubrica::clear(void) {
	_count =0; // riutilizzo la rubrica
...
void rubrica::save(const std::string &nomefile) const {
	...
	if(ofs.is_open()) {
		...
		// Una possibile gestione di errori sul file
		throw errore_file_exception();
	}
